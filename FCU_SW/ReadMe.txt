================================================================================
    MFC 라이브러리 : FCU_SW 프로젝트 개요
===============================================================================

응용 프로그램 마법사에서 이 FCU_SW 응용 프로그램을 만들었습니다. 이 응용 프로그램은 MFC의 기본 사용법을 보여 줄 뿐만 아니라 응용 프로그램을 작성하기 위한 기본 구조를 제공합니다.

FCU_SW 응용 프로그램을 구성하는 각 파일에 대한
요약 설명이 포함되어 있습니다.
...
FCU_SW.h
    ...
    여기에는 resource.h를 비롯한 다른 프로젝트별 헤더가 포함되어 있으며 CFCU_SWApp 응용 프로그램 클래스가 선언되어 있습니다.

FCU_SW.cpp
    이는 응용 프로그램 클래스 CFCU_SWApp가 포함된 기본 응용 프로그램 소스 파일입니다.
...
res\FCU_SW.ico
    아이콘 파일이며, 응용 프로그램의 아이콘으로 사용됩니다. 이 아이콘은 기본 리소스 파일인 FCU_SW.rc에 의해 포함됩니다.
...
FCU_SWDlg.h, FCU_SWDlg.cpp - 대화 상자
    이 파일에는 CFCU_SWDlg 클래스가 포함됩니다. 이 클래스는 응용 프로그램의 주 대화 상자에 대한 동작을 정의합니다. 이 대화 상자 템플릿은 FCU_SW.rc에 있으며, Microsoft Visual C++에서 직접 편집할 수 있습니다.

DlgProxy.h, DlgProxy.cpp - 자동화 개체
    이 파일에는 CFCU_SWDlgAutoProxy 클래스가 포함됩니다. 이 클래스는 자동화 컨트롤러가 대화 상자 엑세스에 사용할 수 있는 자동화 메서드 및 속성 노출을 처리하므로 대화 상자에 대한 자동화 프록시 클래스라고 합니다. 모달 대화 상자 기반 MFC 응용 프로그램의 경우 자동화 개체를 사용자 인터페이스에서 분리하기가 더욱 명확하고 쉽기 때문에 대화 상자 클래스에서 이 메서드 및 속성이 직접 노출될 수 없습니다.
...
StdAfx.h, StdAfx.cpp
    이 파일은 미리 컴파일된 헤더(PCH) 파일 FCU_SW.pch와 미리 컴파일된 형식(PCT) 파일 StdAfx.obj를 빌드하는 데 사용됩니다.
